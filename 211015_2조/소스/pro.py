...
def run(weights=ROOT / 'dataset/widget/yolov5s.pt',  # model.pt path(s)
        source=ROOT / 'dataset/mov',  # file/dir/URL/glob, 0 for webcam
        ...
        project=ROOT / 'dataset/output',  # save results to project/name
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'dataset/widget/yolov5s.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'dataset/output/output.mp4', help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--project', default=ROOT / 'dataset/output/', help='save results to project/name')
    parser.add_argument('--name', default='yolo_v5', help='save results to project/name')
...
capture = cv2.VideoCapture('dataset/mov/3.mp4',)
...
writer = cv2.VideoWriter('dataset/output/output.mp4', codec, fps, (width,height))
writer1 = cv2.VideoWriter('dataset/output/process.mp4', codec, fps, (7680, 3240), isColor=True)
...
def undistort(img, cal_dir='dataset/calibration/wide_dist_pickle.p'):
...
while True:
    ...
    # 왜곡 보정
    img_undist = undistort(img_frame)

    img_frames = img_undist.copy()
    img_frames1 = img_undist.copy()
    img_frames2 = img_undist.copy()
    ...
    for sigma in range(1, 4):
        img_GaussianBlur = cv2.GaussianBlur(img_hls_l, (3, 3), sigma)
        img_GaussianBlurs = cv2.merge((img_GaussianBlur, img_GaussianBlur, img_GaussianBlur))
    ...
    src = np.float32([(0.42, 0.65), (0.495, 0.65), (0.1, 1), (0.75, 1)])
    ...
    img_warp2 = cv2.addWeighted(img_warp1, 1, color_img, 0.4, 0)
    ...
    img_f = cv2.addWeighted(img_frames, 1, img_warp3, 0.4, 0)
    ...
    cont1 = cv2.hconcat([img_frame, img_undist, img_hls_ls, img_GaussianBlurs])
    cont2 = cv2.hconcat([img_sobel_scaleds, sx_binarys, s_binarys, img_binary_addeds])
    cont3 = cv2.hconcat([img_warps, img_warp1, img_warp2, img_warp3])
    cont4 = cv2.vconcat([cont1, cont2, cont3])

    writer.write(img_f)
    writer1.write(cont4)

    cont4 = cv2.pyrDown(cont4)
    cont4 = cv2.pyrDown(cont4)

    img_f = cv2.pyrDown(img_f)
    cv2.imshow('img_f', img_f)
    cv2.imshow('cont4', cont4)
